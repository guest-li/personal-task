...
export default function ScholarshipDetailPage({
  ...
  useEffect(() => {
    if (params?.id) {
      fetch(`/api/v1/admin/scholarships/${params.id}`)
        .then((r) => r.json())
        .then((d) => {
          setForm({
            name: d.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/v1/admin/scholarships/${params.id}`, {
      method: "PUT",
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Scholarship</h1>
      {error && (
